...
// Authenticated fetch — always sends JWT in Authorization header
...
// Fetch a file as a Blob through the auth header (avoids 401 on plain <a> links)
...
// Trigger an authenticated download
async function downloadItem(item) {
  ...
  try {
    ...
    if (btn) {
      btn.disabled  = false;
      btn.innerHTML = '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Download';
    }
  }
}
    ...
    if (tableBody) {
      ...
      if (data.portfolioTable && data.portfolioTable.length > 1) {
        data.portfolioTable.slice(1).forEach(row => {
          ...
          tr.innerHTML = `
            <td style="padding:8px 10px;font-weight:700;border-right:1px solid var(--line);">${escapeHTML(ticker)}</td>
...
async function submitTrade() {
  ...
  if (!ticker || isNaN(shares) || isNaN(price)) { showToast('Please fill out all trade fields', true); return; }
...
function calculateRSI(data, period = 14) {
  ...
  out.push(100 - 100 / (1 + avgGain / (avgLoss || 1)));
...
let chartVixInstance = null, chartS5FIInstance = null, chartRSIInstance = null;
...
async function renderTerminalCharts() {
  ...
  try {
    ...
    const dynamicWidth = Math.max(baseWidth, timestamps.length * 18);
    document.querySelectorAll('.chart-canvas-wrapper').forEach(w => { w.style.width = `${dynamicWidth}px`; });
    ...
    if (diagRsi) {
      diagRsi.textContent = latestRSI ? latestRSI.toFixed(1) : '—';
      diagRsi.style.color = latestRSI > 70 ? 'var(--red)' : latestRSI < 30 ? 'var(--green)' : 'var(--ink)';
    }
    const diagTrend = document.getElementById('diag-trend');
    if (diagTrend) {
      diagTrend.textContent = latestPrice > latestSMA ? 'BULLISH' : 'BEARISH';
      diagTrend.style.color = latestPrice > latestSMA ? 'var(--green)' : 'var(--red)';
    }
    const diagBands = document.getElementById('diag-bands');
    if (diagBands) {
      diagBands.textContent = latestPrice >= latestUpperBand ? 'UPPER BAND TOUCH'
        : latestPrice <= latestLowerBand ? 'LOWER BAND TOUCH' : 'MID-RANGE';
    }
    const diagVix = document.getElementById('diag-vix');
    if (diagVix) diagVix.textContent = latestVix > latestVixSMA ? 'EXPANDING' : 'CONTRACTING';

    // Alerts
    let alertsHTML = '';
    if (latestRSI > 70)
      alertsHTML += `<div style="border-left:3px solid var(--red);padding:8px;font-size:0.6rem;background:var(--paper);border-top:1px solid var(--line);border-right:1px solid var(--line);border-bottom:1px solid var(--line);"><strong>CRITICAL:</strong> SPY Overbought (RSI > 70). Risk of pullback elevated.</div>`;
    else if (latestRSI < 30)
      alertsHTML += `<div style="border-left:3px solid var(--green);padding:8px;font-size:0.6rem;background:var(--paper);border-top:1px solid var(--line);border-right:1px solid var(--line);border-bottom:1px solid var(--line);"><strong>SIGNAL:</strong> SPY Oversold (RSI < 30). Potential bounce forming.</div>`;
    if (latestPrice >= latestUpperBand)
      alertsHTML += `<div style="border-left:3px solid var(--gold);padding:8px;font-size:0.6rem;background:var(--paper);border-top:1px solid var(--line);border-right:1px solid var(--line);border-bottom:1px solid var(--line);"><strong>WARNING:</strong> Price piercing upper Bollinger Band.</div>`;
    if (!alertsHTML)
      alertsHTML = `<div style="padding:12px;border:1px dashed var(--line-bold);font-size:0.55rem;color:var(--ink-dim);text-align:center;text-transform:uppercase;letter-spacing:0.1em;">No active extreme signals</div>`;
    const screener = document.getElementById('screener-results');
    if (screener) screener.innerHTML = alertsHTML;
    ...
    chartVixInstance = new Chart(document.getElementById('chartVix'), {
      type: 'line',
      data: {
        labels:   timestamps,
        datasets: [
    ...
    chartS5FIInstance = new Chart(document.getElementById('chartS5FI'), {
      type: 'line',
      data: {
        labels:   timestamps,
        datasets: [
    ...
    chartRSIInstance = new Chart(document.getElementById('chartRSI'), {
      type: 'line',
      data: {
        labels:   timestamps,
        datasets: [
          { label: '14D RSI',         data: spyRSI,                           borderColor: '#2a4060', borderWidth: 1.5, pointRadius: 0, tension: 0.1 },
          { label: 'Overbought (70)', data: Array(timestamps.length).fill(70), borderColor: '#b01020', borderWidth: 1, borderDash: [2,2], pointRadius: 0 },
          { label: 'Oversold (30)',   data: Array(timestamps.length).fill(30), borderColor: '#2a7a2a', borderWidth: 1, borderDash: [2,2], pointRadius: 0 },
        ],
      },
      options: { ...commonOptions, scales: { ...commonOptions.scales, y: { ...commonOptions.scales.y, min: 10, max: 90 } } },
    });
...
function renderLog() {
  ...
  drops.forEach(item => {
    ...
    // Download — files only, authenticated via blob
    if (item.type === 'file') {
      const dlBtn       = document.createElement('button');
      dlBtn.className   = 'log-btn';
      dlBtn.innerHTML   = `<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Download`;
      dlBtn.addEventListener('click', async function () {
        ...
        try {
          ...
          this.disabled  = false;
          this.innerHTML = `<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Download`;
        }
      });
...
async function loadPublicDrops() {
  ...
  try {
    ...
    publicDrops.forEach(item => {
      ...
      const header     = document.createElement('div');
      header.className = 'log-item-header';
      header.innerHTML = `<span class="log-type-badge file-type">FILE</span>`;
      const nameSpan       = document.createElement('span');
      nameSpan.className   = 'log-name';
      nameSpan.textContent = item.name || '—';
      header.appendChild(nameSpan);
      ...
      const actions     = document.createElement('div');
      actions.className = 'log-actions';
      // Public downloads don't need auth — plain link is fine
      const dlLink       = document.createElement('a');
      dlLink.className   = 'log-btn';
      dlLink.href        = `${API_BASE}/api/drop/public/item/${escapeHTML(item.id)}`;
...
async function previewItem(item) {
  ...
  try {
    // ── URL ───────────────────────────────────────
    if (item.type === 'url') {
      const href = escapeHTML(item.url || item.name || '');
      content.innerHTML = `
        <p style="font-size:0.7rem;margin-bottom:8px;color:var(--ink-dim);">Stored URL:</p>
        <a href="${href}" target="_blank" rel="noopener"
           style="font-size:0.75rem;color:var(--blue);word-break:break-all;">${href}</a>`;
      return;
    }
    ...
    if (item.type === 'text') {
      ...
      content.innerHTML = `<pre style="margin:0;overflow:auto;max-height:65vh;"><code class="language-${lang}">${escapeHTML(text)}</code></pre>`;
      if (window.Prism) Prism.highlightAllUnder(content);
      return;
    }

    // ── File — fetch as authenticated blob ────────
    ...
    if (imageExts.includes(ext)) {
      ...
      img.src           = blobUrl;
      img.alt           = item.name || '';
      img.style.cssText = 'max-width:100%;max-height:65vh;display:block;margin:auto;border:1px solid var(--line);';
      ...
      const blob    = await fetchBlob(item.id);
      const blobUrl = URL.createObjectURL(blob);
      // blobUrl intentionally not revoked here — iframe needs it while open
      content.innerHTML = `<iframe src="${blobUrl}" style="width:100%;height:65vh;border:1px solid var(--line);background:#fff;"></iframe>`;
      ...
      content.innerHTML = `
        <video controls style="max-width:100%;max-height:65vh;display:block;margin:auto;border:1px solid var(--line);">
      ...
      // No preview — offer authenticated download
      content.innerHTML = `
        <div style="text-align:center;padding:32px 0;">
          <p style="font-size:0.65rem;color:var(--ink-dim);text-transform:uppercase;letter-spacing:0.1em;margin-bottom:16px;">
            No preview available for .${escapeHTML(ext)} files.
          </p>
          <button class="btn btn-solid" id="fallback-dl-btn" style="display:inline-flex;align-items:center;gap:6px;">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            Download ${escapeHTML(item.name || '')}
          </button>
        </div>`;
      document.getElementById('fallback-dl-btn').addEventListener('click', async function () {
        this.disabled    = true;
        this.textContent = '…';
        try {
          const blob = await fetchBlob(item.id);
          ...
          this.disabled     = false;
          this.textContent  = `Download ${item.name || ''}`;
        }
      });
    }
  } catch (err) {
...
function closePreview() {
  ...
  // Wiping innerHTML releases any blob URLs held by iframe/video/audio
...
function switchMode(mode) {
  ...
  if (sm) sm.textContent = mode === 'file' ? 'File' : mode === 'text' ? 'Text/Code' : 'URL';
  const pt = document.getElementById('public-toggle-container');
  if (pt) pt.style.display = mode === 'file' ? 'flex' : 'none';
}
...
function setFiles(files) {
  ...
  if (selectedFiles.length === 1) {
    ...
    document.getElementById('file-size-display').textContent = formatBytes(total) + ' Total';
    updateSpec({ type: 'file', name: `Batch: ${selectedFiles.length} files`, size: formatBytes(total) });
  }
}
  ...
  try {
    ...
    if (currentMode === 'file') {
      ...
      successMessage = selectedFiles.length > 1
        ? `${selectedFiles.length} files received`
        : (data.items[0].name || '').substring(0, 32);
      resetFileZone();
...
function resetFileZone() {
  ...
  const fsi = document.getElementById('file-selected-info');
  const fi  = document.getElementById('file-input');
...
async function calculateStorage() {
  try {
    ...
    const txt = document.getElementById('storage-used-text');
    const bar = document.getElementById('storage-bar');
    if (txt) txt.textContent = formatBytes(data.totalBytes) + ' Used';
    if (bar) {
...
function updateSpec(item) {
  if (!item) {
    ['spec-type', 'spec-name', 'spec-size', 'spec-time'].forEach(id => {
      const el = document.getElementById(id);
  ...
  const tm = document.getElementById('spec-time'); if (tm) tm.textContent = item.timestamp
